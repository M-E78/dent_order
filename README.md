...
**App URL:** https://dent-order.com  

**note記事:** https://note.com/m_e_78/n/ncc8b1f38cb1c  

### 【採用ご担当者様へ：テスト用ログイン情報】
動作確認用のテストアカウントをご用意しております。ぜひこちらからログインして、実際のUI/UXをお試しください。

*   **ログインURL:** https://dent-order.com
*   **Eメール:** test@example.com
*   **パスワード:** password123

### 【推奨アクション：音声入力によるセミオート作成のお試し】
「音声によるハンズフリーでの指示書作成」の操作手順については以下のとおりです。手が塞がっている診療中でも素早く正確なメモを残せるよう設計しています。

**操作手順:**
1. ログイン後、「技工指示書の新規作成」画面へ遷移します。
2. 入力欄のマイクボタンを押し、**「右上6番、FMC（エフエムシー）」**と発話してください。
3. 自動で16番（右上6）の歯が選択され、補綴種別に「FMC」がマッピングされる挙動をご確認いただけます。

*(※現在、OpenAI APIを用いた各種パラメータの細かな抽出精度はチューニング中ですが、コアとなる部位抽出と主要な補綴物選択のロジックは実装済みです)*
...
本プロジェクトでは、保守性と信頼性を担保するため、RSpecを用いたテストコードを実装しています。
...
- **担当ドクター選択機能:** 実際の歯科医院の運用フローを考慮し、誰が指示を出したか「責任の所在」を明確にするため、アカウントに紐づくドクターの選択を必須としています。
- **二段階認証（2FA）の実装:** 機微な医療情報や個人情報を扱うプロダクトであるため、Google Authenticator等を用いた2FAを実装しています。
